...
1. ⚠️ **問い合わせフォームの重複質問を削除する**(なお本人の操作が必要)。質問2と質問3が同じラベル「会社名・サービス名・チャンネル名(任意)」の重複で、**ラベルが(任意)なのに質問2だけ必須になっている**。質問2を削除すれば直る
2. ⚠️ **フォームの内容がサイトの売り物と噛み合っていない**(要判断)
   - ご依頼内容の選択肢が「動画用AI画像セット(YouTube・ショート動画)」「SNS縦型AI動画」「広告漫画」など、旧・クリエイター向け案件のまま。サイトが売っている Character Continuity & DLC Kit / Steam Kit / LiveOps がひとつも無い
   - ご予算感の最大が「10万円以上」。サイトの掲載価格は DLC Kit 22万円〜 / Steam Kit 30万円〜 / LiveOps 月額24万円〜 なので、**選択肢の刻み自体が価格アンカーを下げてしまっている**
3. フォームにテスト送信して実際に届くか確認する
4. 人手修正の前後比較2枠(`processBefore` / `processAfter`)の実素材をなおから受け取る。**残る唯一のプレースホルダー**
5. (任意)独自ドメインを当てるかの判断。いまは `*.workers.dev` のサブドメイン
