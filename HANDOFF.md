...
## 📐 レイアウト修正(2026-08-30・2回目)

- **ヘッダーの左端ずれ**: `Header.tsx` だけ左右の余白を内側の `<div>` に付けていて(他は `<header>`/`<section>` 自体に付ける設計)、ブランド名「なお」の左端が本文の見出しより32pxだけ内側にずれていた。paddingを `<header>` へ移して解消(本番実測で56px同士に一致)。モバイルメニューの区切り線(`border-t`)は `-mx-5 sm:-mx-8` で打ち消して画面幅いっぱいを維持
- **お問い合わせの中央寄せ**: `Contact.tsx` だけ共通の `Section` コンポーネントを使わず独自実装(`max-w-3xl` + `text-center`)になっていた。`Section` を使う形に統一し、他の節と同じ左寄せへ。フォーム自体の枠(`max-w-3xl`)は読みやすさのため残している
- ⚠️ **JSXコメント `{/* */}` を `{cond && ( ... )}` の開き括弧の直後に置くと構文エラーになる**(オブジェクトリテラルと解釈される)。このセッションで2回踏んだ。コメントは式の外(`{cond && (` より前)に出すこと
- 金継ぎの卒業候補(散らかり度2/2/2・破れ0)は維持。ずれ度もこの修正で7→5に改善(約物のズレが1件減った副産物)
